...
function loadListNavigator() {
    ...
    function createListLink(name) {
        ...
        listLink.addEventListener("dragstart", function(event) {
            ...
            listDragIndex.initial = Array.from(allListLinks).indexOf(listLink);
        });
        ...
        listLink.addEventListener("drop", function(event) {
            ...
            listDragIndex.new = Array.from(allListLinks).indexOf(listLink);

            const movedList = data[listDragIndex.initial];
            data.splice(listDragIndex.initial, 1);
            data.splice(listDragIndex.new, 0, movedList);

            if (listDragIndex.initial !== listDragIndex.new) {
                selectedListIndex = listDragIndex.new;
            }
    ...
    if (data.length > 0) {
        ...
        html_allListLinks[selectedListIndex].style.backgroundColor = "rgb(150, 170, 200)";
        html_allListLinks[selectedListIndex].style.border = "3px solid gray";
        html_allListLinks[selectedListIndex].style.borderBottom = "3px solid black";
    }
...
{//CSS
    document.addEventListener('mousedown', (event) => {
        ...
        switch (target.id) {
            case "btn-new-list":
                newListName_addingList = true;
                break;
            case "add-task":
                newTaskText_addingTask = true;
                break;
        }
    });
    ...
    document.addEventListener('mouseup', (event) => {
        ...
        switch (target.id) {
            case "btn-new-list":
                newListName_addingList = false;
                break;
            case "add-task":
                newTaskText_addingTask = false;
                break;
        }
    });
    ...
    newListName.value = "Add List";
    newListName.style.color = "gray";
    let newListName_addingList = false;

    newListName.addEventListener('click', () => {
        newListName.value = "";
        newListName.style.color = "black";
    });

    newListName.addEventListener('blur', () => {
        if (!newListName_addingList) {
            onBlur();
        } else {
            checkBlur();
            function checkBlur() {
    ...
    newTaskText.value = "Add Task";
    newTaskText.style.color = "gray";
    let newTaskText_addingTask = false;
    ...
    newTaskText.addEventListener('blur', () => {
        if (!newTaskText_addingTask) {
            onBlur();
        } else {
            checkBlur();
            function checkBlur() {
...
function newList() {
    ...
    if (newListName && newListName !== "Add List") {
        ...
        document.getElementById("new-list-name").value = "Add List";
    }
...
function addTask(text, priority, due_date, due_time) {
    ...
    newTaskContainer.style.height = taskHeight + "px";
    ...
    grabHandle.style.height = taskHeight + "px";
    grabHandle.innerText = "\u{22EE}";

    grabHandle.addEventListener("click", function(event) {
        const target = event.target;
        const taskIndex = findTaskIndex(event.target.parentElement);
        setTimeout(() => {
            const complete = document.createElement("button");
            complete.classList.add("btn", "ctx-menu-btn");
            complete.id = "ctx-btn-taskComplete";
            complete.innerText = list.tasks[taskIndex].complete ? "Mark Incomplete": "Mark Complete";
            complete.addEventListener('click', () => {
                const completionBox = target.parentElement.querySelector('.task-status')
                if (list.tasks[taskIndex].complete) {
                    setTaskStatus(completionBox, false);    
                } else {
                    setTaskStatus(completionBox, true);
                }
            });
    
            const delTask = document.createElement("button");
            delTask.classList.add("btn", "ctx-menu-btn");
            delTask.id = "ctx-btn-del";
            delTask.innerText = "Delete Task";
            delTask.addEventListener('click', () => {
                deleteTask(target.parentElement);
            });

            const editDue = document.createElement("button");
            editDue.classList.add("btn", "ctx-menu-btn");
            editDue.id = "ctx-btn-editDue";
            editDue.innerText = "Edit Due Date";
            editDue.addEventListener('click', () => {
                editDueDate(taskIndex);
            });

            const togNotifs = document.createElement("button");
            togNotifs.classList.add("btn", "ctx-menu-btn");
            togNotifs.id = "ctx-btn-togNotifs";
            togNotifs.innerText = list.tasks[taskIndex].dueNotifs ? "Disable Notifications": "Enable Notifications";
            togNotifs.addEventListener('click', () => {
                toggleDueNotifs(taskIndex);
            });
            ...
            orderInput.value = `Change order (${taskIndex + 1})`;
            orderInput.style.color = 'gray';
            orderInput.id = "ctx-btn-order-input";
            orderInput.style.height = '30px';

            let orderInputIsOpen = false;
            orderInput.addEventListener('click', () => {
                if (orderInputIsOpen) {
                    return;
                }
                
                orderInputIsOpen = true;

                orderInput.type = 'number';
                orderInput.value = '';
                orderInput.max = list.tasks.length;
                orderInput.min = 1;
                orderInput.style.color = 'black';

                const interval_checkEnter = setInterval(checkEnter, 10);
            
                function checkEnter() {
                    if (keysDown.enter) {
                        editOrder(removeInterval);
                        closeCtxMenu();
                    }
                }

                function removeInterval() {
                    clearInterval(interval_checkEnter);
                }
            });
            
            const order = document.createElement("button");
            order.classList.add("btn", "ctx-menu-btn");
            order.id = "ctx-btn-order";
            order.innerText = "Confirm Order";
            order.addEventListener('click', () => {
                editOrder();
            });

            function editOrder(callback) {
                const removedElement = list.tasks.splice(taskIndex, 1)[0];
                list.tasks.splice(document.getElementById('ctx-btn-order-input').value - 1, 0, removedElement);
                messageConsole("Task Moved.");
            ...
            menu.classList.add("ctx-menu", "task-ctx-menu");
    
            menu.appendChild(complete);
            menu.appendChild(delTask);
            menu.appendChild(editDue);
            menu.appendChild(togNotifs);
            menu.appendChild(hr);
            menu.appendChild(orderInput);
            menu.appendChild(order);
    
            const menuWidth = 150;
            const menuHeight = menu.childElementCount * 40 - 40;
            menu.style.width = `${menuWidth}px`;
            menu.style.height = `${menuHeight}px`
            
            menu.style.left = `${returnCtxMenuPosition(event.clientX, event.clientY, menuWidth, menuHeight).x}px`;
            menu.style.top = `${returnCtxMenuPosition(event.clientX, event.clientY, menuWidth, menuHeight).y}px`;
    ...
    newTaskStatus.style.height = taskHeight + "px";
    ...
    */
    ...
    deleteButton.src = "./assets/x.png"
    deleteButton.alt = "X";
    deleteButton.draggable = false;
    deleteButton.classList.add("btn");
    deleteButton.classList.add("task-delete");
    deleteButton.style.width = 50 + "px";
    deleteButton.style.height = taskHeight + "px";
...
function handleInput(event) {
    ...
    if (document.querySelector(".task-ctx-menu")) {
        closeCtxMenu();
        return;
    }
...
function closeTaskEditing(override) {
    if (!override && !confirm("Are you sure you want to exit editing? ('Shift + Button click' will do this without confirmation.)")) {
        return;
    }
...
function openListCtxMenu(x, y, target) {
    ...
    listSettingsBtn.classList.add("btn", "ctx-menu-btn");
    listSettingsBtn.id = "ctx-btn-list-settings";
    listSettingsBtn.innerText = "Settings";
    ...
    menu.appendChild(listSettingsBtn);
    ...
    html_listCtxMenu.addEventListener("click", function(event) {
        ...
        switch (target.id) {
            ...
            case "ctx-btn-del-list":
                deleteList(index);
                break;
            case "ctx-btn-list-settings":
                openListSettings(index);
                break;
        }
    });
}

function openListSettings(index) {
    const menu = document.createElement("div");
    menu.classList.add("list-edit-menu");

    //Head
    const listSettingsHeadText = document.createElement("div");
    listSettingsHeadText.id = "list-settings-head";
    listSettingsHeadText.innerText = "List Settings";

    const btn_cancel = document.createElement("button");
    btn_cancel.classList.add("btn")
    btn_cancel.id = "btn-close-list-setting";
    btn_cancel.innerText = "Cancel";

    const listSettingsHead = document.createElement("div");
    listSettingsHead.classList.add("list-settings-head");
    listSettingsHead.appendChild(listSettingsHeadText);
    listSettingsHead.appendChild(btn_cancel);
...
function openDefaultCtxMenu(x, y) {
    ...
    about.classList.add("ctx-menu-about");
    about.id = "ctx-about";
    about.innerText = version.semantic;
    about.style.height = "40px";
...
function request_addTask() {
    ...
    document.getElementById("new-task-text").value = ""; //Clear Input

    if (newTaskText === "Add Task") {
        newTaskText = "";
        document.getElementById("new-task-text").value = "Add Task";
    }
...
function updateLocalStorage() {
    //data[selectedListIndex] = list; LINE BREAKS LIST DRAG AND DROP... FOR SOME REASON
